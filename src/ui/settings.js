...
export function renderDocumentSettings(s, transposeInfo = null) {
  return t`<h2 class="panel-title"><span>El documento</span><span class="muted">01</span></h2>
    ...
    <div id="document-options-content">
      ...
      <label class="footer-option"><input id="showBrand" type="checkbox" ${s.showBrand !== false ? "checked" : ""}><span>Mostrar chordleaf.com en el pie</span></label>
      <div class="music-controls"><div><label>TRANSPORTAR</label><div class="stepper"><button id="transpose-down" aria-label="Bajar intervalo">−</button><select id="transpose-interval" aria-label="Intervalo de transposición" title="Intervalo de transposición"><option value="1">½ tono</option><option value="2">1 tono</option><option value="4">2 tonos</option></select><button id="transpose-up" aria-label="Subir intervalo">＋</button></div></div><button id="link" class="chain ${s.linked ? "linked" : ""}" aria-label="Vincular cejilla y acordes" aria-pressed="${s.linked}" title="${s.linked ? t("Mantener la tonalidad que suena") : t("La cejilla solo cambia la indicación")}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="m10 14 4-4m-6 6-1 1a4 4 0 0 1-6-6l4-4a4 4 0 0 1 6 0m2 2 1-1a4 4 0 0 1 6 6l-4 4a4 4 0 0 1-6 0" transform="translate(1 -1)"/></svg></button><div><label>CEJILLA</label><div class="stepper"><button id="capo-down" aria-label="Bajar cejilla">−</button><input id="capo" type="number" min="0" max="12" value="${s.capo}" aria-label="Cejilla"><button id="capo-up" aria-label="Subir cejilla">＋</button></div></div></div>
      ${transposeInfo ? `<p class="link-help" role="status">${esc(transposeInfo.label)} · ${esc(transposeInfo.before)} → ${esc(transposeInfo.after)} <button id="undo-transpose" type="button">${t("Deshacer transposición")}</button></p>` : ""}
