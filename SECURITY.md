...
- Local documents are processed in the browser. Saved website HTML and pasted content are read only at the user’s request, within file/text budgets, and stay on the device. DOMPurify restricts HTML to the text structure and metadata needed by the provider parsers. Its inert document fragment is never mounted as a live webpage; embedded scripts and resources are not executed or loaded.
