...
If a chord has no catalog position, Chordleaf asks you to define its six frets or add only the available diagrams. It never prints a “No position” placeholder inside the block. You can edit a missing position later from its song card; once saved, existing **All chords** blocks include it automatically.
