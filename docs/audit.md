...
**Updated:** September 21, 2026. Baseline: `39bf11b`, plus the production-readiness changes.

[← Back to Chordi](../README.md) · [Deployment guide](deployment.md) · [Security policy](../SECURITY.md)

## Current decision

The application is suitable for a protected review deployment. The original code now has the owner-approved MIT license. English and Spanish are supported, while repository documentation remains English. Public launch and repository visibility are separate owner decisions.

Vercel's first deployment was automatically assigned to production. **All project deployments are now protected with Vercel Authentication**, including the production alias. An unauthenticated request redirects to sign-in. No custom domain has been selected.

## Remaining priorities

| Priority | Action before the relevant release                                         | Evidence / limitation                                                                                                                                                                                                                                                                 |
| -------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1       | Complete hosted CI, including Firefox, and review the final preview.       | Local Chromium and WebKit workflows pass. The downloaded Firefox binary cannot find its profile on this Mac; CI runs all three engines on Linux.                                                                                                                                      |
| P1       | Decide when to make the repository and website public.                     | The GitHub repository remains private. Branch protection is unavailable under the current private-repository account plan; enable required checks after publication or a suitable plan change. No paid upgrade is needed for the protected review.                                    |
| P1       | Review provider availability and usage before enabling public web imports. | The Vercel firewall is active at 10 requests/IP/minute; a bounded test returned ten 503 responses from the disabled endpoint, then two 429 responses. Production imports remain disabled; the protected test preview can enable them. Provider access may differ from local behavior. |
| P1       | Perform human accessibility and real-device acceptance checks.             | Automated Chromium/WebKit coverage and earlier axe checks do not replace a real iPhone, keyboard-at-200%-zoom and screen-reader session.                                                                                                                                              |
...
| P2       | Continue profiling unusually large or hostile documents.                   | Auto-fit runs in a terminable worker; DOCX declared expansion is bounded. The DOCX decoder and PDF processing are not a complete hostile-document sandbox. Low-end phone profiling remains useful.                                                                                    |
| P2       | Consolidate remaining UI render helpers and CSS incrementally.             | The static shell was extracted; unnecessary settings/chord-panel work during lyric edits was reduced. Broad CSS restructuring needs visual regression coverage rather than blind deletion of overrides.                                                                               |
...
| Secret review              | Gitleaks 8.30.1, downloaded from its official release and checksum-verified: 11 reachable commits and the current working directory scanned with redacted output, no findings. This is not a guarantee that every possible secret or ownership issue is absent. |
...
- Additional Chromium and WebKit workflow checks passed: exclusive editing, takeover, persistence, backup restore and language switching.
- Earlier frozen install and dependency audit: zero reported advisories; rerun in hosted CI.
...
- Optional export chunks still trigger Vite's size warning. The final main bundle remains roughly 65 kB gzip; large PDF/DOCX modules load on demand.
